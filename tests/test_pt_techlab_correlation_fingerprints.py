...
def test_cross_source_host_and_hash_produce_equal_candidates() -> None:
    ...
    assert correlation_fingerprints(edr, rules) == correlation_fingerprints(ndr, rules)
    assert len(correlation_fingerprints(edr, rules)) == 2
...
def test_hash_rule_correlates_independently_of_host() -> None:
    ...
    assert correlation_fingerprints(first, rules) == correlation_fingerprints(second, rules)
...
def test_invalid_or_incomplete_rules_do_not_create_keys() -> None:
    ...
    assert correlation_fingerprints(event, rules) == []

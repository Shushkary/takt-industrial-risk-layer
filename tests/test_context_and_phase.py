...
def _event(*, op: str, asset: str = "plc-1") -> NormalizedEvent:
    return NormalizedEvent(
        event_id="e1",
        observed_at=datetime(2026, 5, 6, 14, 0, tzinfo=UTC),
...
def test_context_no_tickets_non_critical_neutral_score():
    m = match_event_to_ticket(_event(op="PING"), [], now=datetime.now(UTC))
...
def test_tag_phase_work_shift_weekday_utc():
    ts = datetime(2026, 5, 6, 10, 30, tzinfo=UTC)
    label = tag_phase(ts)
...
def test_tag_phase_night_weekday_utc():
    ts = datetime(2026, 5, 6, 22, 0, tzinfo=UTC)
    label = tag_phase(ts)
...
def test_tag_phase_respects_explicit_zone():
    ts = datetime(2026, 5, 6, 21, 0, tzinfo=UTC)
...
def test_tag_phase_work_shift_hour_boundaries_weekday():
    ...
    assert tag_phase(datetime(2026, 5, 6, 7, 59, tzinfo=UTC)).phase == WorkPhase.NIGHT
    assert tag_phase(datetime(2026, 5, 6, 8, 0, tzinfo=UTC)).phase == WorkPhase.WORK_SHIFT
    assert tag_phase(datetime(2026, 5, 6, 19, 59, tzinfo=UTC)).phase == WorkPhase.WORK_SHIFT
    assert tag_phase(datetime(2026, 5, 6, 20, 0, tzinfo=UTC)).phase == WorkPhase.NIGHT

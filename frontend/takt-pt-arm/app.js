...
const HELP = {
  ...
  sim_summary: {
    ...
    source: 'Всё перечисленное собрано по событиям этого кейса: класс риска — из оценки кейса, правила — из каталога правил продукта, варианты реагирования — из артефактов кейса с признаком отличительной сущности.',
  ...
  invariants: {
    ...
    what: 'Правила детектирования ТАКТ, сработавшие на событиях кейса. Срабатывание — признак отклонения, а не вердикт. В документации продукта такое правило называется инвариантом.',
  ...
  assemble_auto: {
    ...
    what: 'Собирает связанные инциденты из дел очереди: берёт дела, где сработало правило, и по их отличительным сущностям отбирает все события этой цепочки — из всех источников, а не только из одного дела.',
    ...
    action: 'Кнопка нужна, чтобы пересобрать с другим порогом отличительности — например, когда атака растянута во времени и её сущности встречаются чаще порога. Инцидент с тем же составом сущностей остаётся тем же кейсом; редакция, которую пересборка не подтвердила, закрывается, если вы её ещё не разбирали.',
    doc: 'docs/pt_techlab/correlation_quality.md',
  },
  assemble_threshold: {
  ...
  assistant: {
    ...
    action: 'При несогласии с выводом идти по шагам сверху вниз и находить тот, на котором расходится: состав дела правится на шаге сборки, неопределённость снимается на шаге сверки приложением документа, ошибка правила — на шаге проверки правил. Что здесь могла бы делать языковая модель и чего не делает — отдельное пояснение «Роль языковой модели» на вкладке «Симуляция».',
    doc: 'docs/product_boundary.md',
  },
  training: {
...
// Подписи — словами аналитика, а не обозначениями модели: `rhythm` показывается как
// «частота возникновения», `graph` — как «связи». Ключи остаются кодами модели: их
// принимает продукт, и перевод в запросе означал бы второй словарь на стороне окна.
const WEIGHT_FIELDS = [
  ['rhythm', 'Частота возникновения'],
  ['graph', 'Связи'],
  ['context', 'Контекст'],
  ['user', 'Учётная запись'],
  ...
  try {
    ...
    if (!incidents.length) {
      ...
      toast(considered
        ...
        : 'Собирать не из чего: дел со сработавшим правилом нет');
    } else {
...
function openStep(order) {
  ...
  const paragraphs = [
    ...
    detection.invariants && detection.invariants.length
      ...
      : 'Правила на этом событии не срабатывали: оно попало в кейс по связи сущностей, а не по признаку правила.',

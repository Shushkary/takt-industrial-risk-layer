...
test('operator shell renders in explicit local demo mode without active control actions', async ({ page }) => {
  ...
  await expect(page.getByText('Compliance выключен')).toBeVisible()
...
test('queue row can be selected with keyboard and opened as a case for operator decision', async ({ page }) => {
  ...
  await expect(page.getByText(/локальное демо-решение оператора NON-EVIDENCE/i)).toBeVisible()
})
...
test('manual permit and formal verdict are recorded through the case workflow', async ({ page }) => {
  ...
  await expect(page.getByText(/локальный демо-наряд NON-EVIDENCE.*WO-E2E-0001/i)).toBeVisible()
  ...
  await expect(page.getByText(/локальный демо-вердикт NON-EVIDENCE/i)).toBeVisible()
})

...
assert.match(appShell, /Compliance включён/, 'app shell must show global compliance badge')
...
assert.match(queue, /Compliance mode блокирует локальные демо-отчёты очереди без backend API/, 'incident queue must block local demo reports in compliance mode')
...
assert.match(caseDetail, /Проверить серверный forensic ZIP/, 'case detail must expose forensic ZIP verification control')
...
assert.match(caseDetail, /NON-EVIDENCE LOCAL OPERATOR NOTE/, 'local case export must be marked as non-evidence')
...
assert.match(caseDetail, /серверный forensic ZIP/, 'local case UI must direct evidence use to server forensic ZIP')
assert.match(caseDetail, /Печать\/PDF NON-EVIDENCE/, 'local print/PDF action must be visibly marked as non-evidence')
assert.match(caseDetail, /локальное демо-решение оператора NON-EVIDENCE/, 'local demo operator decisions must be marked as non-evidence')
assert.match(caseDetail, /локальный демо-наряд NON-EVIDENCE/, 'local demo manual permits must be marked as non-evidence')
assert.match(caseDetail, /локальный демо-вердикт NON-EVIDENCE/, 'local demo formal verdicts must be marked as non-evidence')
...
for (const [name, source] of [
  ...
  assert.match(source, /NON-EVIDENCE LOCAL UI REPORT/, `${name} local report must be marked as non-evidence`)
...
assert.match(invariants, /Compliance mode блокирует локальные отчёты по инвариантам/, 'invariant library must block local reports in compliance mode')
assert.match(invariants, /disabled=\{localReportBlocked\}/, 'invariant report controls must be disabled in compliance mode')
assert.match(invariants, /backend-каталог не отображается/, 'invariant library must not claim local fallback is shown after API errors')
...
assert.match(topology, /не влияет на рабочий backend-граф/, 'topology must explain that sliders do not affect API graph data')
...
assert.match(settings, /Compliance mode блокирует локальные отчёты настроек/, 'settings audit must block local settings reports in compliance mode')
assert.match(settings, /disabled=\{localReportBlocked\}/, 'settings audit report controls must be disabled in compliance mode')
assert.match(settings, /Локальное подтверждение изоляции NON-EVIDENCE/, 'settings local air-gap confirmation must be visibly marked as non-evidence')
...
assert.match(settings, /Локальные UI-отчёты остаются NON-EVIDENCE/, 'settings audit must label local reports as non-evidence near ledger checks')

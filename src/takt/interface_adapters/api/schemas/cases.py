...
class CaseSummary(BaseModel):
    ...
    # Разобрано ли это дело в составе собранного инцидента. Связь ведёт собранный инцидент
    # (`related_cases`), обратной ссылки у исходного дела нет — и в очереди оно выглядит
    # нетронутым, хотя его событие уже разобрали. Считает продукт: по загруженной странице
    # очереди покрытие вышло бы неполным.
    covered_by_case_id: str = ""
    """Собранный инцидент, в который вошли события этого дела; пусто — не вошли ни в один."""

    covered_by_status: str = ""
    """Статус того инцидента: разобран он или ещё ждёт очереди."""

    covered_events: int = 0
    """Сколько событий этого дела вошло в собранный инцидент."""

    coverage: str = ""
    """`full` — вошли все события дела, `partial` — часть, пусто — покрытия нет.

    Русские названия — в `GET /catalog/vocabulary`, таблица `case_coverage`."""
...
class CaseGroupOut(BaseModel):
    ...
    который оно вошло: два инцидента и сумма их событий там, где событие одно."""

...
_L2_SUFFIXES: tuple[str, ...] = (
    ...
    "/split",
    "/events/attach",
    # Утверждение итогового описания: именно утверждённая редакция уходит в доказательный
    # пакет. Писать редакцию может первая линия — это рабочая запись; предъявлять её как
    # итог расследования решает вторая, как и решение по делу.
    "/summary/approve",
)

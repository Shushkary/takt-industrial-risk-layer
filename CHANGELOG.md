...
### Порог отличительности сборки — параметр, а не константа

Значение по умолчанию (`distinctive_max_events: 12`) откалибровано на корпусе INC-002
и было документировано как замер, но не как проверка на генерализацию. Проверено
независимым внешним корпусом (`EXT-001`, AIT Alert Data Set, CC-BY-4.0, не синтетика
продукта): атака растянута на 18 часов, узлы и адреса естественно встречаются 18–37 раз,
и на пороге 12 (вплоть до 30) сборка не даёт **ни одного** инцидента — хотя все 208
событий фикстуры размечены одной цепочкой.

Один глобальный порог не может одновременно быть безопасным для короткой синтетической
атаки и достаточным для многочасовой: значение, связывающее EXT-001 (от 40), на
смешанном корпусе (EXT-001 рядом с INC-002) уже на 40 разрушает чистоту результата
INC-002 — цепочка начинает захватывать фон (24 события цепочки вместе со 187 фоновыми,
было 23 без единого фонового). Это причина оставить порог параметром, а не искать одно
число, которое подойдёт всем: он уже был доступен через `--distinctive-max-events` у
`load_dataset` и `distinctive_max_events` в теле `POST /cases/assemble/auto`, но не был
доступен из АРМ — добавлено поле «порог отличительности» рядом с кнопкой «Собрать
инциденты», с честным пояснением компромисса.

Замер: `tests/test_auto_assemble_incidents_ext001.py`. Разбор с полной таблицей —

...
### Расширение до узла из АРМ (второй шаг сборки пивотом)

`POST /cases/assemble/pivot` умел `expand_hosts` и был проверен, но вызывался только
утилитой командной строки (`docs/pt_techlab/analyst_window.md`, раздел 2): аналитик,
собравший инцидент через АРМ, второй шаг разбора выполнить не мог, не уходя в консоль.

На карточке дела, собранного пивотом (автоматически или вручную), новый блок
«Расширение до узла»: список — узлы, встретившиеся среди событий этого дела, уже
расширенные отмечены. Действие вызывает тот же эндпоинт с теми же сидами (читаются из
уже сохранённых `case.artifacts`, `source: pivot-seed` — не набираются заново) и
выбранными узлами. Backend не менялся: чистая проводка существующей, уже протестированной
возможности до интерфейса.

На корпусе INC-002 расширение автоматически собранного инцидента (23 события, вся цепочка
без фона) до узлов `ws-17` и `build-srv-01` добирает 2 из 4 недостающих события цепочки —
39 событий, 25 из 27 цепочки, 14 фоновых (ожидаемая плата за полноту, отсев остаётся за
аналитиком). Оставшиеся два события несут не физический узел, а объект конвейера в поле
`host_id` (`artifact:app-setup.msi`, `pipeline:release-prod`) — задокументированный формат
сида (`--seed host:artifact:...`), но автоматический подбор такого сида на этом корпусе не
проверить: ни одно срабатывающее событие цепочки не делит с ними это значение. Не включён.

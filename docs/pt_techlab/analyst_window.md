...
| Показатель | Значение |
|---|---|
| Кейс INC-002 | 41 событие, класс MEDIUM, балл 0.513 |
...
Почему MEDIUM, а не выше: маркеры векторов «ритм» и «контекст» в
`risk_vectors_from_invariants` — промышленные инварианты (опрос Modbus, физика процесса), и
SOC-инцидент их не поднимает; вектор качества данных при полных данных тоже даёт ноль. Из
пяти векторов срабатываниями SOC поднимаются два — граф и пользователь, их веса в сумме 0.40
при пороге «высокого» 0.65. Остальное добирается измерениями (ритм 0.75, контекст 0.45),
которые сборка переносит из вошедших дел.

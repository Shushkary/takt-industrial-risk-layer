...
берётся и что с этим делать. Реестр пояснений — константа `HELP` в `app.js`, 50 записей, из
них 41 со ссылкой на документ-источник. Запись имеет вид `{title, what, source, action, doc}`

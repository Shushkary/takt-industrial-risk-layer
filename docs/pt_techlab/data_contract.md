...
| Процесс | `process_guid`, `parent_process_guid` | `subject.process.guid`, `subject.process.parent.guid`, `.name`, `.cmdline` (пароль из неё вычищается) | — (в схеме отсутствует: сетевой сенсор процессов не видит) | `tag` как технологический процесс/точка |

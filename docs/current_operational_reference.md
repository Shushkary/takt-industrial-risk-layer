...
| Объём backend-прогона | порядка 1750 тестов, ~6 мин на машине разработчика | точное значение — вывод `python -m pytest` |
| Память контейнера API на боевом стенде | 47,85 МиБ | замер `docker stats takt-api`; условия — ВМ 2 vCPU / 985 МиБ, 44 дела в базе, зафиксирован 2026-09-13 |
...
Замер памяти — единственная строка таблицы, которую не держит тест: это наблюдение на стенде, а не
инвариант кода. Число привязано к объёму базы и поедет вместе с ним, поэтому во внешних материалах
его называют только вместе с условиями. Повторить замер:

```bash
sudo docker stats --no-stream --format "{{.Name}} {{.MemUsage}}" takt-api
```

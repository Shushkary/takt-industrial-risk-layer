...
| Однопользовательская модель (нет ролей) | **Не реализовано.** Один статический ключ на все write-операции; `request_actor` пишет заявленного actor, но не проверяет права | См. промпт v0.8, Блок 4 (RBAC) — открытый пункт |
...
2. **Отсутствие ролей/RBAC** — один статический `TAKT_API_KEY` на все write-операции; невозможно различить
   оператора, аудитора и администратора на уровне API (промпт v0.8, Блок 4).
...
5. **Ingest только push/REST** — нет прямого приёма из PT ISIM/MaxPatrol SIEM (syslog/CEF); при интеграции
   с PT это отдельная поверхность атаки (доверие к приёмнику, разбор CEF) — вне объёма этого документа до
   появления кода интеграции (промпт v0.8, Блок 1).
...
- Идентификация и аутентификация субъектов доступа: частично (см. п. 5.2 — нет ролей).

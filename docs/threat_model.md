...
4. **TPR/FPR-протокол частичный** — `docs/detection_quality.md` покрывает 11 из 26 правил синтетическим
   baseline-корпусом (не промышленной выборкой); остальные 15 и независимая проверка на реальном трафике
   остаются открытым пунктом (промпт v0.8, Блок 3).
...
- Обнаружение вторжений/аномалий: 26 задекларированных правил, 19 реально активных в проде (раздел 4.4,
  `docs/invariant_matrix.md`); TPR/FPR baseline для 11 из 26 на синтетическом корпусе (`docs/detection_quality.md`),
  без независимого статистического подтверждения на промышленном трафике.

...
TPR/FPR-протоколы (Блок 3 промпта v0.8, `docs/detection_quality.md`) на момент записи отсутствуют ни для одного инварианта:
`docs/product_boundary.md` и `docs/certification_risk_roadmap.md` фиксируют это как открытый пункт ФСТЭК-пакета.
...
2. Собрать эталонные датасеты «без атак» + размеченные сценарии атак и посчитать TPR/FPR по каждому инварианту
